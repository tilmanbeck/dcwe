...
def main():
    ...
    # progressive setup
    start_bin = 1
    for i in range(start_bin, int(max(dataframe[partition].values))):
        progressive_output_dir = os.path.join(output_dir, str("dev_bin_") + str(i))
        if not os.path.exists(progressive_output_dir):
            os.makedirs(progressive_output_dir)

        # current bin = dev bin
        # all bins before dev bin -> train bins
        # all bins after dev bin -> test bins (up until len(test_bins)==1)
        train_bins = [x for x in dataframe[partition].unique() if x < i]
        dev_bin = [float(i)]
        test_bins = [x for x in dataframe[partition].unique() if x > i]
        ...
        training_args = TrainingArguments(
            output_dir=progressive_output_dir,          # output directory
...
#            callbacks=[EarlyStoppingCallback(early_stopping_patience=args.early_stopping_patience)],
        ...
        with open(os.path.join(progressive_output_dir, 'eval_results.json'), 'w') as fp:
        ...
        with open(os.path.join(progressive_output_dir, 'training_args.json'), 'w') as fp:
